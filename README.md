...
```bash
Веб приложение на стеке HTML + CSS + JavaScript для вычисления контрольой md5 суммы файла
```
...
3. перетянуть файл в область выделенную пунктиром
   ...
   будет отображена вычисленная контрольная сумма
   см. скриншоты ниже;

4. для проверки вычисленного md5 хэша можно воспользоваться олайн инструментом "MD5 Hash Generator":
...
```bash
В процессе разработки веб-приложения, появилась идея разместить статическое значение вычесленного md5 хэша страницы index.html 
на этойже странице, чтобы при вычислении хэша этой страницы путем перебрасывания файла, 
отображаемый и вычисленный ранее хэш совпадал с ткущим вычесленным.
...
но невозможно сделать вычесленный md5 частью того для чего вычисляется md5 хэш.

В источнике ниже приведено обсуждение этой математической аномолии:
